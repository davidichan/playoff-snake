  ...
  output$SnakePlot <- renderPlotly({ 
    if(input$comp == F){
      ...
      trad_diff <- plot_ly(data = current.records.clean[[t_name]], x = ~GP, 
        y = ~(Points - (input$pace/82) * GP), mode = "lines", name = '2017-18 differential', type = 'scatter', mode = 'lines', line = list(color = 'red', width = 3)) %>% 
        add_trace(y = ~(P2017 - (input$pace/82) * GP), name = '2016-17 differential', line = list(color = 'blue', width = 2)) %>% 
        add_trace(y = ~(P2016 - (input$pace/82) * GP), name = '2015-16 differential', line = list(color = 'purple', width = 2), visible = "legendonly") %>% 
        add_trace(y = ~(P2015 - (input$pace/82) * GP), name = '2014-15 differential', line = list(color = 'green', width = 2), visible = "legendonly") %>% 
      ...
      t_name <- paste0(input$chtype, "2018")
      t2_name <- paste0(input$compteam, "2018")
      print(t2_name)
      #trad <- ggplot(data= season2017.records.clean[[t_name]], aes(x=GP, y=Points)) + geom_line(colour="red", size=1) + geom_line(aes(x=GP, y=P2016), colour="purple4", size=0.75) + geom_line(aes(x=GP, y=P2015), colour="forestgreen", size=0.75) + geom_line(aes(x=GP, y=Snake), size=0.75) + snake_labs + snake_lims
      trad <- plot_ly(data = current.records.clean[[t_name]], x = ~GP, 
                      y = ~Points, mode = "lines", name = paste(input$chtype, '2017-18 Season'), type = 'scatter', mode = 'lines', line = list(color = 'red', width = 3)) %>% 
        add_trace(y = ~P2017, name = paste(input$chtype, '2016-17 Season'), line = list(color = 'pink', width = 2), visible = "legendonly") %>% 
        add_trace(data = current.records.clean[[t2_name]], y = ~Points, name = paste(input$compteam, '2017-18'), line = list(color = 'green', width = 2)) %>% 
        add_trace(y = ~P2017, name = paste(input$compteam, '2016-17'), line = list(color = 'olive', width = 2), visible = "legendonly") %>% 
        add_trace(y = ~(Snake/96)*input$pace, name = '96 Point pace', line = list(color = 'black', width = 2), showlegend = F) %>%
        layout(xaxis = list(title = "Games Played"), yaxis = list (title = "Points", range = c(0,110)))
      
      trad_diff <- plot_ly(data = current.records.clean[[t_name]], x = ~GP, 
                           y = ~(Points - (input$pace/82) * GP), mode = "lines", name = paste(input$chtype, '2017-18 differential'), type = 'scatter', mode = 'lines', line = list(color = 'red', width = 3)) %>% 
        add_trace(y = ~(P2017 - (input$pace/82) * GP), name = paste(input$chtype, '2016-17 differential'), line = list(color = 'pink', width = 2), visible = "legendonly") %>% 
        add_trace(data = current.records.clean[[t2_name]], y = ~(Points - (input$pace/82) * GP), name = paste(input$compteam,'2017-18 differential'), line = list(color = 'green', width = 2)) %>% 
        add_trace(y = ~(P2015 - (input$pace/82) * GP), name = paste(input$compteam, '2016-17 differential'), line = list(color = 'olive', width = 2), visible = "legendonly") %>% 
        layout(xaxis = list(title = "Games Played"), yaxis = list (title = "Point diff. to snake", range = c(-15,15))) %>% 
        add_trace(x = c(0, 82), y = c(0, 0), line=list(color='black', width=2), name = 'Snake', showlegend = F)
      subplot(trad, trad_diff, nrows = 2, shareX = T, titleX = T, titleY = T)
    }
